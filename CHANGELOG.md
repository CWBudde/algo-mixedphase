...
- `DesignContinuum` designs for a **requested group delay in samples**, which is the
  quantity a latency-constrained caller actually has. It is the only entry point whose
  parameter is measured in the units the constraint is expressed in; the existing five
  expose a mix, a latency budget or a magnitude tolerance, and two of them need a sweep
  before the caller learns what a given latency costs.

  Two results make it exact rather than a search. The prescribed phase of
  `DesignPhaseInterpolation` is linear in its mix and group delay is a linear functional of
  phase, so the realised weighted mean delay is affine in the mix,
  `tau(mix) = (1-mix)*tau_min + mix*(Length-1)/2`, which inverts in closed form. And because
  a mix is confined to `[0, 2]`, the same relation bounds what phase alone reaches to
  `[tau_min, Length-1-tau_min]` — a window the requested magnitude sets rather than the tap
  count, spanning 127.0 samples for the reference room correction and 29.3 for the LR8
  crossover in the same 129 taps. Measured against the six reference targets the realised
  delay tracks the request to within 0.28 samples, the projection error of the affine law.

  Outside that window the roles of the Wu–Gao–Teo objective and constraint are exchanged:
  `lowDelayProblem.evaluateMatchDelay` minimises magnitude error subject to the requested
  delay instead of delay subject to a magnitude band, reusing the same L-BFGS minimiser and
  penalty ladder. On the reference low-pass this reaches 1.46 samples at 16.0% relative
  magnitude error and 0.58 dB RMS, where `DesignLowGroupDelay` at its published 2 dB
  tolerance stops at 1.76 samples with 22.8% and 1.90 dB. On the LR8 crossover a request of
  37.0 samples improves on minimum-phase truncation in latency **and** in both magnitude
  measures at once, because the floor there guards a magnitude 129 taps cannot realise.

  `DesignLowGroupDelay` and every other existing entry point are byte-for-byte unchanged,
  which `TestDesignLowGroupDelayIsUnchangedByTheNewProblemForm` and `just compare-check`
  both gate. `Result` gains `Regime` and `AchievedGroupDelay`; the new `ErrDelayOutOfReach`
  rejects a request outside `[0, Length-1]`.
- `docs/reference-continuum.csv` and `docs/reference-continuum-impulse.csv`, generated by
  `just compare` and byte-gated by `just compare-check`. The first carries one requested
  delay per row across both branches, with `predicted_delay` beside `mean_group_delay` so
  the residual of the affine law is a committed number rather than a claim. The second
  records the low-pass at six positions along the continuum, which is what makes the
  reversal into maximum phase visible instead of asserted.
- `just paper-brief` builds the short companion account, and CI now format-checks and
  publishes it alongside the full paper.

### Changed

- The paper is rewritten around the requested-delay parameter, with a new title and
  abstract; the brief is rewritten as a short version of the same story so the two cannot
  disagree. Seven new figures and both numeric tables are generated from the committed CSVs
  at build time, replacing the two hand-typed tables of the previous draft. Three
  measured results are reported that the previous framing had no place for: group-delay
  ripple falls monotonically to below 1e-13 samples at the centre of each window; the
  phase-pure ends of a window are its **most accurate** points as well as its fastest and
  slowest, by a factor of 1.6e7 on the LR4 crossover; and both structures vanish on a target
  whose magnitude does not fit its support, which the window width warns about in advance.
- `examples/mixedphase` takes its artifact paths as a struct rather than a positional
  parameter list, which had grown to seven strings.
